...
function repl(){
  var code, cont, rl, reset, prompt, that, vm, ref$, server, jscomplete, camelize, uncamelize, endCapLetter, complete, _ttyWrite;
  argv[1] = 'repl';
  code = repl.infunc ? '  ' : '';
  cont = 0;
  ...
  if (!o.compile) {
    ...
    jscomplete = bind$(server, 'complete');
    camelize == null && (camelize = function(s){
      return s.replace(/-[a-z]/g, function(it){
        return it.charAt(1).toUpperCase();
      });
    });
    uncamelize = function(s){
      return s.replace(/[a-z][A-Z]/g, function(it){
        return it.charAt(0) + "-" + it.charAt(1).toLowerCase();
      });
    };
    endCapLetter = function(s){
      return s.replace(/-$/, "[A-Z]");
    };
    complete = function(line, cb){
      var camelLine, nodashes;
      camelLine = camelize(line);
      nodashes = camelLine.replace(/-$/, "");
      return jscomplete(nodashes, function(err, completions){
        var ref$, lineRegex;
        if (!err && !completions[0].length && line.match("-")) {
          return jscomplete(line, function(err, completions){
            return cb(err, completions);
          });
        } else {
          if (line.match("-") && (completions != null && ((ref$ = completions[0]) != null && ref$.map)) && completions[0].length) {
            lineRegex = /./;
            try {
              lineRegex = new RegExp("^(" + endCapLetter(line) + "|" + endCapLetter(camelLine) + ")");
            } catch (e$) {}
            try {
              completions[0] = completions[0].filter(function(it){
                return it.match(lineRegex);
              }).map(uncamelize);
            } catch (e$) {}
            completions[1] = line;
          }
